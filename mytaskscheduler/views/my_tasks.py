...
from ..models import save_tasks, load_tasks, MY_TASKS_FILE
from ..email_utils import send_email_reminder, check_due_tasks

bp = Blueprint('my_tasks', __name__, url_prefix='/mytasks')

# Load tasks from file
def get_my_tasks():
    return load_tasks(MY_TASKS_FILE)

def save_my_tasks(tasks):
    save_tasks(tasks, MY_TASKS_FILE)
...
def my_dashboard():
    ...
    save_my_tasks(all_tasks)
            
    return render_template('my_dashboard.html', tasks=filtered_tasks)
...
def add_task():
    if request.method == 'POST':
        ...
        tasks.append(task)
        save_my_tasks(tasks)
        return redirect(url_for('my_tasks.my_dashboard'))
    return render_template('add_my_task.html')
